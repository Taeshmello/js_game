...
const render = Render.create({
    ...
    options: {
        wireframes : false,
        background: "linear-gradient(143deg, #ace2f1, #00c2da, #006e91, #003f55, #001e28, black)", // 배경
        width: 620,
        height: 1000,
    }
});
...
const leftWall = Bodies.rectangle(15, 610, 30, 2000, {
    isStatic : true, //고정기능
    render : {fillStyle : "#38EDDB"}
});

const rightWall = Bodies.rectangle(605, 610, 30, 2000, {
    isStatic : true, //고정기능
    render : {fillStyle : "#38EDDB"}
});

const ground = Bodies.rectangle(310, 1000, 620, 60,{
    isStatic : true,
    render : {fillStyle: "#38EDDB"}
});
...
// 소리를 재생하는 함수
const playSound = async (url, duration) => {
    const audio = new Audio("falling.mp3");
    audio.loop = false;
    audio.volume = 0.5;
    await audio.play();
...
window.onkeydown = async (event) => {
    ...
    switch(event.code) {
        ...
        case "KeyS":
            ...
            disableAction = true;
            await playSound("falling.mp3", 1500);
            addFruit();
            disableAction = false;
            break;
    }
